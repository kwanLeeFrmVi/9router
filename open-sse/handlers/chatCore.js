...
export async function handleChatCore({
  ...
  if (!clientRequestedStreaming && providerRequiresStreaming) {
    ...
    if (isSSEResponse) {
      const isResponsesApi = targetFormat === "openai-responses";
      ...
      if (isResponsesApi) {
        ...
        try {
          ...
          log?.info?.(
            "STREAM",
            `Converted Responses API SSE → JSON for non-streaming client`,
          );
          ...
          // translate the Responses API JSON back to Chat Completions format
          let finalResponse = jsonResponse;
          if (sourceFormat !== "openai-responses") {
            const textContent = (jsonResponse.output || [])
            ...
            finalResponse = {
              id: jsonResponse.id
                ? jsonResponse.id.replace(/^resp_/, "chatcmpl-")
                : `chatcmpl-${Date.now()}`,
              object: "chat.completion",
              ...
              choices: [
                {
                  ...
                  finish_reason:
                    jsonResponse.status === "completed" ? "stop" : "stop",
                },
              ],
              usage: usage
                ? {
